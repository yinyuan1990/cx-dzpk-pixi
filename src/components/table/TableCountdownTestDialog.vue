...
// Countdown-effect test: pick a seat position + duration, start the timer ring effect.
...
const style = ref('ring') // ring = 德州经典进度环；fuse = 牛仔放炮导火索
...
<template>
  <transition name="fade">
    <div v-if="show" class="mask" @click.self="emit('close')">
      <div class="panel">
        <div class="head">
          <span class="title">{{ t('table.countdownTest') }}</span>
        ...
        <div class="label">{{ t('table.countdownSec') }}</div>
        ...
        <div class="label">{{ t('table.countdownStyle') }}</div>
        <div class="grid grid2">
          <button class="num" :class="{ on: style === 'ring' }" @click="style = 'ring'">
            {{ t('table.countdownStyleRing') }}
          </button>
          <button class="num" :class="{ on: style === 'fuse' }" @click="style = 'fuse'">
            {{ t('table.countdownStyleFuse') }}
          </button>
        </div>

        <button class="test-btn" @click="emit('test', { seat, seconds, style })">
          {{ t('table.startCountdown') }}
        </button>
      </div>
    </div>
  </transition>
