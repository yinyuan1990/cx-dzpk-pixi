...
// Dealer button + blinds (#3) test: pick the dealer seat; SB = dealer+1, BB = dealer+2.
// Seats 9 players (if needed), places the D button, posts SB/BB blind chips.
...
const sbAmount = ref(500)
const bbAmount = ref(1000)
</script>
...
<template>
  <transition name="fade">
    <div v-if="show" class="mask" @click.self="emit('close')">
      <div class="panel">
        <div class="head">
          <span class="title">{{ t('table.blindTest') }}</span>
          <button class="close" data-sound="back" @click="emit('close')">×</button>
        </div>

        <div class="label">{{ t('table.dealerSeat') }}</div>
        <div class="grid">
          <button
            v-for="n in SEATS"
            :key="n"
        ...
        <div class="hint">SB = {{ (dealer + 1) % 9 }} · BB = {{ (dealer + 2) % 9 }}</div>

        <button
          class="test-btn"
          @click="emit('test', { dealer, sbAmount, bbAmount })"
        >
          {{ t('table.startTest') }}
        </button>
      </div>
